...
/**
 * Provider-agnostic token usage record. All fields optional because each
 * provider exposes a different subset. We capture whatever's available and
 * persist it to the runs log so usage / spend can be audited later.
 */
export interface LlmUsage {
  /** Tokens consumed by the input prompt + system + cached context. */
  inputTokens?: number;
  /** Tokens generated by the model. */
  outputTokens?: number;
  /** Anthropic cache-creation token count (Claude Code). */
  cacheCreationInputTokens?: number;
  /** Anthropic cache-read token count (Claude Code). */
  cacheReadInputTokens?: number;
  /** Ollama: end-to-end duration including queueing, in milliseconds. */
  totalDurationMs?: number;
  /** Ollama: pure generation time in milliseconds. */
  evalDurationMs?: number;
  /** Ollama: prompt evaluation time in milliseconds. */
  promptEvalDurationMs?: number;
  /** Claude Code: cost in USD the CLI reports for the call. */
  costUsd?: number;
}
...
interface GenerateOpts {
  ...
  numCtx?: number;
  signal?: AbortSignal;
  /** Called exactly once when token usage for the call becomes available.
   *  Cloud providers (AI SDK) emit after the stream completes; Ollama and
...
export async function llmGenerateOnce(opts: GenerateOpts): Promise<string> {
  ...
  if (opts.onUsage) {
    const u = result.usage as
      ...
      | undefined;
    if (u) {
      opts.onUsage({ inputTokens: u.inputTokens, outputTokens: u.outputTokens });
    }
  }
  return result.text;
}
...
export async function* streamLlmGenerate(
  ...
  if (opts.onUsage) {
    try {
      ...
      /* AI SDK didn't surface usage — skip silently */
    }
  }
}
